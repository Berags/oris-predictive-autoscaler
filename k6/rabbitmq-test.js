...
console.log(
  `Test distribution: ${DISTRIBUTION}, λ=${PARAM_ARRAY_JSON} arrivals/sec, duration: ${TEST_DURATION}`
);
...
export const genericProcess = () => {
  ...
  for (let i = 0; i < PARAM_ARRAY.length; i++) {
    stateDistributions.push(
      DistributionFactory.getFromType(DISTRIBUTION, PARAM_ARRAY[i])
    );
  }
  ...
  while (true) {
    ...
    const now = Date.now();
    const compensatedWaitTime = computeCompensatedWaitMs(
      now,
      nextScheduledTime,
      publishOverhead
    );
    ...
    if (overheadSamples < MAX_OVERHEAD_SAMPLES) {
      ...
      publishOverhead = emaOverhead(
        overheadSamples === 0 ? undefined : publishOverhead,
        actualOverhead,
        alpha
      );
      overheadSamples++;
    ...
    const recentEffectiveRate =
      computeRecentEffectiveRate(publishTimestamps, 100) ||
